...
struct poly{
    ...
    // asegura sentido horario, elimina colineales, rota el vector para que pt[0] sea el primero
	void normalize() { 
		delete_collinears();
	...
	bool is_convex() { // delete collinear points first O(n)
		int N = sz(pt);
		if(N < 3) return false;
    ...
    // busca el punto mas lejano en una direccion dada
