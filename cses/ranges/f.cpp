...
ll queryST(int v, int tl, int tr, int l, int r){
   ...
   int tm = (tl + tr)/2;
   ll m = queryST(v*2, tl, tm, l, min(r, tm)) + queryST(v*2+1, tm+1, tr, max(l, tm+1), r); // esto lo hace para sumar
   //cout << l << " " << r << " " << m << endl;
   return m;
...
int main(){
    ...
    forn(i, q){
        ...
        if(p == 2){
            int pos;
            cin >> pos;
            cout << queryST(1, 0, mult-1, 0, pos-1) << endl;
        }
        if(p == 1){
            int a, b, k;
            ll valb;
            cin >> a >> b >> k;
            ll vala = queryST(1, 0, mult-1, a-1, a-1);
            if(b!=n)valb = queryST(1, 0, mult-1, b, b);
            updateST(1, 0, mult-1, a-1, vala+k);
            if(b!=n)updateST(1, 0, mult-1, b, valb-k);
        } 
    }
    return 0;
}