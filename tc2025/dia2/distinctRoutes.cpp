...
vector<int> aux;
bool visitado[504][504];
void dfs(int i){
    if(i == nodes-1)return;
    for(auto j: G[i]){
        if(j.f>0 && !visitado[i][j.to]){
            visitado[i][j.to] = true;
            aux.push_back(j.to+1);
            dfs(j.to);
            break;
        }
    }
...
int main() {
    ...
    forn(i, m){
        int a , b;
        cin >> a >> b;
        addEdge(a-1,b-1,(ll)1);
    }
    ll flow = maxFlow(0, nodes-1);
    vector<vector<int>> v;
    for(auto j: G[0]){
        if(j.f>0){
            aux.clear();
