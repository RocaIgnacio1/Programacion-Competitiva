...
int main(){
    ...
    while (t--) {
        int n,d,h;
        cin >> n >> d >> h;
        double area = double(d)*double(h) / 2.0;
        ...
        */
        ...
        forn(i,n){
            ans+=area;
            if(i!=0 && y[i]-y[i-1]<h){
                double pendiente = (d/2.0/h);
                double in = (y[i-1]+h-y[i])*pendiente;
                ans -= in*(y[i-1]+h-y[i]);
                /*
                cout << fixed << setprecision(9) << pendiente << endl;
                cout << fixed << setprecision(9) << in << endl;   
                cout << y[i-1] << " " << h << " " << y[i] << endl;
                */
            }
        }
